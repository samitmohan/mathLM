"""Generate synthetic calculus Q&A pairs → math_qa.bin.

Covers: power rule, coefficient + polynomial derivatives, second derivatives,
indefinite integrals, trig, exp/log, chain rule, product rule, basic arithmetic.

    python -m mathlm.data.generate_calculus
    python -m mathlm.data.generate_calculus --check   # print 20 samples
"""
...
from mathlm.model.tokenizer import MathTokenizer as _MathTokenizer
if _MathTokenizer.is_available():
    _math_tok = _MathTokenizer(); _math_tok.load()
    tokenise = _math_tok.encode
else:
    import tiktoken
