...
class GPTConfig:
    ...
    gradient_checkpointing: bool = False # trade recompute for memory; enables larger batch sizes or bigger models
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, cos, sin, layer_idx, kv_cache=None):
        ...
        # KV cache (inference) — store compact heads BEFORE expanding, so the cache stays small
        if kv_cache is not None:
            key, value = kv_cache.update(layer_idx, key, value, max_seq_len=self.max_seq_len)

        # expand KV heads to match Q heads (GQA: 4 KV heads → 8 Q heads)
...
class MLP(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.gate = nn.Linear(config.embedding_dim, hidden_dim, bias=False) # SwiGLU gate: element-wise multiplied with fc1 output to control information flow
...
class SparseMOE(nn.Module):
    '''
    Sparse Mixture of Experts (MoE) feedforward layer.
    Instead of one MLP processing all tokens, we have N expert MLPs and route each token
    to the top-2 experts (like Mixtral). Only those 2 experts run per token — sparse, not all N.
    A load-balancing loss penalises routing collapse (all tokens going to the same expert).
    '''
    def __init__(self, config):
        super().__init__()
...
class GPT(nn.Module):
    ...
    def forward(self, input_ids, target_ids=None, kv_cache=None):
        ...
        for block in self.blocks:
            if self.config.gradient_checkpointing and self.training:
                # kv_cache is always None during training; pass None explicitly so checkpoint
                # doesn't need to differentiate through the cache update path
                hidden_states = torch.utils.checkpoint.checkpoint(
                    block, hidden_states, cos, sin, None, use_reentrant=False
                )
            else:
    ...
    def stream_generate(self, token_ids, max_new_tokens, temperature=1.0, top_k=None, top_p=None):
        ...
        kv_cache = KVCache(len(self.blocks))
        # Prefill: run the full prompt through the model to populate the KV cache
        logits, _ = self(token_ids, kv_cache=kv_cache)

        for _ in range(max_new_tokens):
            next_token = self._sample_next_token(logits, temperature, top_k, top_p)
            yield next_token.item()
            # Decode: feed only the new token; KV cache holds all prior context.
            # KVCache.update() already trims itself to max_seq_len so no manual reset needed.
            logits, _ = self(next_token, kv_cache=kv_cache)
